...
},{"./fromevent":3,"./transposition":7,"./virtual-hyperscript":9,"matches-selector":22,"rx":25,"vdom-parser":30,"virtual-dom/diff":37,"virtual-dom/patch":38}],6:[function(require,module,exports){
"use strict";
...
},{"./transposition":7,"rx":25,"vdom-to-html":34}],7:[function(require,module,exports){
"use strict";
...
},{"rx":25,"virtual-dom/vnode/vnode":57}],8:[function(require,module,exports){
/*eslint-disable */
'use strict';
...
// END Cycle.js-specific code <<<<<<<<<<
},{"./virtual-hyperscript":9,"virtual-dom/virtual-hyperscript/hooks/attribute-hook":45,"virtual-dom/virtual-hyperscript/svg-attribute-namespace":49,"x-is-array":62}],9:[function(require,module,exports){
/* eslint-disable */
'use strict';
...
/* eslint-enable */
},{"virtual-dom/virtual-hyperscript/hooks/ev-hook.js":46,"virtual-dom/virtual-hyperscript/hooks/soft-set-hook.js":47,"virtual-dom/virtual-hyperscript/parse-tag.js":48,"virtual-dom/vnode/is-thunk":51,"virtual-dom/vnode/is-vhook":52,"virtual-dom/vnode/is-vnode":53,"virtual-dom/vnode/is-vtext":54,"virtual-dom/vnode/is-widget":55,"virtual-dom/vnode/vnode.js":57,"virtual-dom/vnode/vtext.js":59,"x-is-array":62}],10:[function(require,module,exports){
"use strict";
...
function intent(DOM) {
  var changeColor$ = DOM.select('.color-field').events('input').map(function (ev) {
    return { color: ev.target.value };
  });
  var changeWidth$ = DOM.select('.width-slider').events('input').map(function (ev) {
    return { width: parseInt(ev.target.value) };
  });
  var destroy$ = DOM.select('.remove-btn').events('click').map(function (ev) {
    return true;
  });

  return { changeColor$: changeColor$, changeWidth$: changeWidth$, destroy$: destroy$ };
}

function model(props, actions) {
  var color$ = props.color$.take(1).startWith('#888').concat(actions.changeColor$.map(function (_ref) {
    var color = _ref.color;
    return color;
  }));
  var width$ = props.width$.take(1).startWith(200).concat(actions.changeWidth$.map(function (_ref2) {
    var width = _ref2.width;
    return width;
  }));

  return (0, _rxCombineLatestObj2.default)({ color$: color$, width$: width$ });
}
...
function view(state$) {
  return state$.map(function (_ref3) {
    ...
    return (0, _dom.div)('.item', { style: style }, [(0, _dom.input)('.color-field', {
      type: 'text',
      attributes: { value: color }
    }), (0, _dom.div)('.slider-container', [(0, _dom.input)('.width-slider', {
      type: 'range', min: '200', max: '1000',
      attributes: { value: width }
    })]), (0, _dom.div)('.width-content', String(width)), (0, _dom.button)('.remove-btn', 'Remove')]);
  });
}
...
},{"@cycle/core":1,"@cycle/dom":2,"rx-combine-latest-obj":24}],65:[function(require,module,exports){
'use strict';
...
function intent(DOM, itemActions) {
  var addItem$ = _rx.Observable.merge(DOM.select('.add-one-btn').events('click').map(function () {
    return 1;
  }), DOM.select('.add-many-btn').events('click').map(function () {
    return 10;
  }));
  var removeItem$ = itemActions.destroy$;
...
function model(actions, itemFn) {
  ...
  function createNewItem(props) {
    var id = mutableLastId++;
    var sinks = itemFn(props, id);
    sinks.DOM = sinks.DOM.replay(null, 1);
    sinks.DOM.connect();
    sinks.destroy$ = sinks.destroy$.publish();
    sinks.destroy$.connect();
    return { id: id, DOM: sinks.DOM, destroy$: sinks.destroy$ };
  }

  var initialState = [createNewItem({ color: 'red', width: 300 })];

  var addItemMod$ = actions.addItem$.map(function (amount) {
    var newItems = [];
  ...
  var removeItemMod$ = actions.removeItem$.map(function (id) {
    return function (listItems) {
  ...
  return _rx.Observable.merge(addItemMod$, removeItemMod$).startWith(initialState).scan(function (listItems, modification) {
...
function view(itemDOMs$) {
  function renderTopButtons() {
    return (0, _dom.div)('.topButtons', [(0, _dom.button)('.add-one-btn', 'Add New Item'), (0, _dom.button)('.add-many-btn', 'Add Many Items')]);
  }

  return itemDOMs$.map(function (itemDOMs) {
    return (0, _dom.div)('.list', [renderTopButtons()].concat(itemDOMs));
  });
}

function makeItemWrapper(DOM) {
  return function itemWrapper(props, id) {
    var propsObservables = {
      color$: _rx.Observable.just(props.color),
      width$: _rx.Observable.just(props.width)
    };
    var item = (0, _isolate2.default)(_Item2.default)({ DOM: DOM, props: propsObservables });
    return {
      DOM: item.DOM,
      destroy$: item.destroy$.map(function () {
        return id;
      })
    };
  };
...
function List(sources) {
  var itemActions = { destroy$: new _rx.Subject() };
  var actions = intent(sources.DOM, itemActions);
  var itemWrapper = makeItemWrapper(sources.DOM);
  var items$ = model(actions, itemWrapper);
  var itemDOMs$ = items$.map(function (items) {
    return items.map(function (item) {
      return item.DOM;
    });
  });
  var itemDestroy$ = items$.filter(function (items) {
    return items.length;
  }).flatMapLatest(function (items) {
    return _rx.Observable.merge(items.map(function (item) {
      return item.destroy$;
    }));
  });
  itemDestroy$.subscribe(itemActions.destroy$.asObserver());
  var vtree$ = view(itemDOMs$);
...
},{"./Item":64,"@cycle/dom":2,"@cycle/isolate":10,"rx":25}],66:[function(require,module,exports){
'use strict';
...
},{"./List":65,"@cycle/core":1,"@cycle/dom":2}]},{},[66]);
